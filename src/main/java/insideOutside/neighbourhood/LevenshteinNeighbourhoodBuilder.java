...
public class LevenshteinNeighbourhoodBuilder extends NeighbourhoodBuilder {
    ...
    public LevenshteinNeighbourhoodBuilder(List<Sequence> negatives, int distanceLimit, boolean random) {
    ...
    public List<Sequence> build(Sequence positive, int maxSize) {
        ...
        List<Sequence> potentialNeighbours = negatives.keySet().stream()
                ...
                .filter(s -> distanceService.levenshtein(positive.symbolList(), s.symbolList()) <= distanceLimit)
